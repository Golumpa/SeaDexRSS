...
def format_snapshot_data(snapshot):
    ...
    formatted_output = f"""
<a href="https://releases.moe/{snapshot.anilist_id}/"><img src="{snapshot.cover_image_url}" alt="{snapshot.anime_title}" style="float: left; margin-right: 10px;"/>
<h2>There has been an update for the SeaDex entry for {snapshot.anime_title}.</h2></a>
...
<ul>
    ...
    <li><strong>Is Best?:</strong> <span style="color: {'green' if torrent.get('isBest') else 'red'};">{torrent.get('isBest', 'False')}</span></li>
    <li><strong>Dual Audio?:</strong> <span style="color: {'green' if torrent.get('dualAudio') else 'red'};">{torrent.get('dualAudio', 'False')}</span></li>
...
def generate_rss(snapshots, anilist_id):
    ...
    for snapshot in snapshots:
        ...
        fe.link(href=f"https://releases.moe/{anilist_id}/")
